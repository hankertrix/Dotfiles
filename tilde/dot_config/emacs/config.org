...
** Large file warning
Set the large file warning threshold to 24MiB, as my PDF files are often larger than 10MB but less than 24MiB.
#+begin_src emacs-lisp
...
*** Org babel support for mermaid CLI
#+begin_src emacs-lisp
(use-package ob-mermaid

  ;; Ensure that mermaid CLI is installed
  :ensure-system-package (mmdc . "sudo pacman -S mermaid-cli")

  ;; Set the mermaid-cli path
  :custom (ob-mermaid-cli-path "/usr/bin/mmdc")

  ;; Load the mermaid language
  :config (org-babel-do-load-languages
           'org-babel-load-languages
           (append org-babel-load-languages
                   '((mermaid . t)))))
