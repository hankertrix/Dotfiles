...
General.el has to be loaded first so that the ~use-package~ definitions below it can make use of the ~:general use-package~ keyword. Below are the key binds for built-in Emacs packages and modes.
#+begin_src emacs-lisp
(use-package general
...
** Wgrep
Wgrep is augments the ~grep~ buffer in Emacs so that it becomes writable, and changes to the buffer will affect the files as well. It is similar to the [[https://github.com/stevearc/quicker.nvim][quicker.nvim]] plugin for the Neovim quick fix list.
#+begin_src emacs-lisp
(use-package wgrep)
#+end_src
